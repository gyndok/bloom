...
## Complete mobile library

All 54 allowlisted PDFs have static Bloom readers. The original PDFs remain available for printing and completing forms. The 51 additional conversions retain all extracted source-word tokens before the explicitly recorded clinician corrections; print timestamps, bare pagination, and local file URLs are omitted from the text. A per-page original-layout WebP retains diagrams and forms. Source review dates are reported only when present. This is not a fresh clinical review of every pamphlet.

`python scripts/convert-guide-library.py` regenerates the added readers and source-page assets using PyMuPDF with Pillow. Three manually structured guides are preserved. The converter groups text by source cards and columns, associates bullet markers, and supplies explicit labeled rows for the source’s weight, glucose, rash comparison, and welcome schedule tables. Source hashes and the conversion audit are checked in. Navigation imports only the lightweight guide ID index; full content is passed per reader route.
